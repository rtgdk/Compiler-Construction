...
AStree parseToAST(parsetree PT){
	...
	if(flag==0) {
	    //printf("Returnign Null from parsetoast for %s",ast->name);
...
int isOperator(AStree ast){
    if(strcmp(ast->name,"PLUS")==0 || strcmp(ast->name,"MINUS")==0 || strcmp(ast->name,"MUL")==0 || strcmp(ast->name,"DIV")==0){
        return 1;
    }
    else{
        return 0;
