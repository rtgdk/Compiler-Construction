...
//			else{
			...
			if(newnode==NULL){
				...
				printf("\n%d: Syntax Error: The token '%s' for lexeme '%s' does not match at line %d. The expected token here is one of <%s>.",tk.lineno,tk.name,tk.lexeme,tk.lineno,expectedTk);
				error++;
				//error recovery
				...
				if (synchMatrix[hashNode->ruleNode][i]==1){
					printf("Found synch %s",tk.name);
					pop(&st);
					ParseTree = next(ParseTree);
				}
				else{
					while(tk.type!=EOF1 && followMatrix[hashNode->ruleNode][i]==0){
						tk = getNextToken(fp);
						for(i=0;i<NO_OF_TERMINALS;i++){
							if(strcmp(tk.name,terminals[i])==0) break;
						}
					...
					pop(&st);
					ParseTree = next(ParseTree);
					printf("Found match for %s",tk.name);
				}
//					tk = getNextToken(fp);
//					int flag=1;
