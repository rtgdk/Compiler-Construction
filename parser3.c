	...
	if(hashNode==NULL){
		...
		//	printf("Checking for rule %d",i);
			...
			//	printf("Checking for %s matching with %s",temp->name,name);
				if(strcmp(temp->name,name)==0){
					//printf("%s matched with %s in rulle no %d -- next is %s",temp->name,name,i,temp->next->name);
					if(temp->next==NULL || temp->next->type==2) {// don't check for A->A recursion
						if(strcmp(grammer[i]->head->name,temp->name)==0){
							break;
						}
					//	printf("GOing for First");
						findfollow(grammer[i]->head->name);
						hashtable* hashNode2;
						hashNode2 = present(grammer[i]->head->name);
						for (i=0;i<NO_OF_TERMINALS;i++){
							if(followMatrix[hashNode2->ruleNode][i]==1){
								followMatrix[hashNode->ruleNode][i]=1;
							}
						}
					}
					else {
						//printf("%s-->" ,temp->name);
						 ...
						 if (hashNode2==NULL){
							...
							if (firstMatrix[hashNode2->ruleNode][39]==1){  //NO_OF_TERMINALS-1
								...
								for (i=0;i<NO_OF_TERMINALS;i++){
									if(followMatrix[hashNode3->ruleNode][i]==1){
										followMatrix[hashNode->ruleNode][i]=1;
									}
								}
							}
