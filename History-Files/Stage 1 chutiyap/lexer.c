...
// void error(int lineNumber, char lexeme[100], int lenLex, char str[100])
// {
//     lexeme[lenLex]='\0';
//     printf("Error in LineNumber:%d at Lexeme:%s\n%s\n",lineNumber,lexeme,str);
//     exit(0);
// }
		...
		switch(state)
		{
			case 1: //start
				switch(ch){
					...
					//else if(buf[index]=='$'){tok.lexeme[i++]=buf[index];index++;state=81;}
					case '$': tk.type = EOF1; tk.lexeme[lexemecount++] ='\0'; tk.name="EOF";return tk;
