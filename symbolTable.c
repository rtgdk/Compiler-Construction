...
SymbolTablePtr createNewST(function f){
	...
	temp->start = 0;
	temp->end = 0;
	variablenodeptr temp1 = f.inputList;
	while(temp1 != NULL)
	{
		addVariable(temp1->v,temp);
		temp1 = temp1->next;
	}
	temp1 = f.outputList;
	while(temp1 != NULL)
...
/*
create to variable from PT's data
*/
variable convertNodeToVariable(parsetree PT){
	variable v;
	strcpy(v.name,PT->tk.lexeme);
	v.linedec = PT->tk.lineno;
	v.assigned = false;
	v.type = 0;
...
/*
 createa varaible pointer from PT's data
*/
variablenodeptr createVptrFromNode(parsetree PT){
	variablenodeptr vptr = (variablenodeptr)malloc(sizeof(variablenode));
	strcpy(vptr->v.name,PT->tk.lexeme);
	vptr->v.linedec = PT->tk.lineno;
...
*/
void addInputList(parsetree PT, function *f)
{
	int i=0;
	if(PT->ruleNode->type==0 && (PT->tk.type == ID))
	{
		int vtype;
		if(strcmp(PT->parent->ruleNode->name,"<parameter_list>")==0)
			vtype = getType(PT->parent->child[0]);
		else
			vtype = getType(PT->parent->child[1]);
		variablenodeptr vptr = createVptrFromNode(PT);
		vptr->v.type = vtype;
...
/*
Add PT's variable PT ptr to symboltable st
*/
void addVariableID(parsetree PT, int type, SymbolTablePtr st)
{
	if(PT->ruleNode->type==0 && PT->tk.type == ID)
	{
		variable v= convertNodeToVariable(PT);
		v.type = type;
		if(type==4){
			v.row = 0;
			v.col = 0;
		...
		//printf("Adding variable%s to%s",v.name,currentTable->f.name);
		addVariable(v, st);
		return;
	}
	int i;
	for(i = 0; i < PT->noc ; i++)
		addVariableID(PT->child[i],type,st);
}
...
*/
void addOutputList(parsetree PT,function *f)
{
	int i;
	if(PT->ruleNode->type==0 && (PT->tk.type == ID))
	{
